(function () {
  ...
  function createPopup() {
    ...
    if (volumeSlider) {
      ...
      try {
        const savedVolume = localStorage.getItem(`${WIDGET_NAMESPACE}-volume`);
        if (savedVolume !== null) {
          volume = parseFloat(savedVolume);
        }
      } catch (e) {
        console.log("Could not retrieve saved volume");
      }

      volumeSlider.value = volume;

      // Add touch event listeners for mobile devices with special handling for iOS
      volumeSlider.addEventListener(
        "touchstart",
        function (e) {
          e.stopPropagation();
          // Prevent default behavior on iOS to avoid conflicts with native controls
          if (isIOS()) {
            e.preventDefault();
          }
        },
      ...
      // Special handling for iOS input event
      if (isIOS()) {
        volumeSlider.addEventListener("input", function (e) {
          // iOS sometimes doesn't trigger the value change correctly
          // Force update the volume when input event fires
